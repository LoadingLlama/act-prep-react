...
/**
 * EMERGENCY CHECK - TEST 2 DATABASE STATUS
...
async function checkTable(tableName, sectionName, expectedCount) {
  ...
  const { data, error } = await supabase
    ...
    .select('*')
    .eq('test_number', TEST_NUMBER)
    .order('question_number' in tableName ? 'question_number' : 'passage_number')
    .limit(3);
...
const englishPassagesOK = await checkTable('act_english_passages', 'English Passages', 5);
const readingPassagesOK = await checkTable('act_reading_passages', 'Reading Passages', 4);
const sciencePassagesOK = await checkTable('act_science_passages', 'Science Passages', 6);
