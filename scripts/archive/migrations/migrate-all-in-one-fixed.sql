-- ============================================================
-- ALL-IN-ONE MIGRATION: Create Tables & Migrate Data (FIXED)
...
-- Step 5: Migrate Data from old lessons table
-- Insert lesson metadata (FIXED: proper column references)
INSERT INTO lesson_metadata (lesson_key, title, subject, category, difficulty_level, duration_minutes, order_index, is_published)
SELECT
  l.lesson_key,
  l.title,
  COALESCE(l.subject, 'general') as subject,
  COALESCE(l.category, 'General') as category,
  COALESCE(l.difficulty, 1) as difficulty_level,
  COALESCE(l.duration, 30) as duration_minutes,
...
-- Insert lesson sections (one main section per lesson with all content)
...
-- Insert section content (the actual HTML content)
