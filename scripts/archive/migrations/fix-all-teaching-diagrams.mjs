/**
 * Fix ALL Teaching Diagrams - Use Generator for Perfect Accuracy
 * Rule 1, Rule 2, and Parallel Lines Section
 */
...
// RULE 1: Vertical Angles Teaching Diagram
function generateRule1VerticalAngles() {
  const cx = 250;
  const cy = 150;
  const lineLength = 80;
  const angle = 70; // Use 70° for clear acute/obtuse distinction
  ...
  // Arcs for all 4 angles with BLUE for acute (70°) and RED for obtuse (110°)
  const arcRadius = 22;
  ...
  // Acute angle 3 (top left) - BLUE (vertical to angle 1)
  const arc3Start = polarToCartesian(cx, cy, arcRadius, 180);
  const arc3End = polarToCartesian(cx, cy, arcRadius, 180 - angle);

  // Obtuse angle 4 (bottom left) - RED (vertical to angle 2)
  const arc4Start = polarToCartesian(cx, cy, arcRadius, 180 - angle);
  const arc4End = polarToCartesian(cx, cy, arcRadius, 360);

  // Label positions
  const label1Pos = polarToCartesian(cx, cy, 40, -angle / 2);
  const label2Pos = polarToCartesian(cx, cy, 40, (-angle + 180) / 2);
  const label3Pos = polarToCartesian(cx, cy, 40, 180 - angle / 2);
  const label4Pos = polarToCartesian(cx, cy, 40, (180 - angle + 360) / 2);
  ...
  return `<svg width="500" height="300" viewBox="0 0 500 300" xmlns="http://www.w3.org/2000/svg" style="background: white; max-width: 100%; height: auto;">
    ...
    <!-- Acute angle 3 (blue) - vertical to 1 -->
    ...
    <!-- Obtuse angle 4 (red) - vertical to 2 -->
    ...
    <!-- Labels -->
    <circle cx="${label1Pos.x.toFixed(1)}" cy="${label1Pos.y.toFixed(1)}" r="18" fill="#3b82f6"/>
    <text x="${label1Pos.x.toFixed(1)}" y="${label1Pos.y.toFixed(1)}" font-family="Arial, sans-serif" font-size="16" font-weight="bold" fill="white" text-anchor="middle" dominant-baseline="middle">a</text>

    <circle cx="${label2Pos.x.toFixed(1)}" cy="${label2Pos.y.toFixed(1)}" r="18" fill="#ef4444"/>
    <text x="${label2Pos.x.toFixed(1)}" y="${label2Pos.y.toFixed(1)}" font-family="Arial, sans-serif" font-size="16" font-weight="bold" fill="white" text-anchor="middle" dominant-baseline="middle">b</text>

    <circle cx="${label3Pos.x.toFixed(1)}" cy="${label3Pos.y.toFixed(1)}" r="18" fill="#3b82f6"/>
    <text x="${label3Pos.x.toFixed(1)}" y="${label3Pos.y.toFixed(1)}" font-family="Arial, sans-serif" font-size="16" font-weight="bold" fill="white" text-anchor="middle" dominant-baseline="middle">a</text>

    <circle cx="${label4Pos.x.toFixed(1)}" cy="${label4Pos.y.toFixed(1)}" r="18" fill="#ef4444"/>
    <text x="${label4Pos.x.toFixed(1)}" y="${label4Pos.y.toFixed(1)}" font-family="Arial, sans-serif" font-size="16" font-weight="bold" fill="white" text-anchor="middle" dominant-baseline="middle">b</text>
</svg>`;
}

// RULE 2: Adjacent Angles Teaching Diagram
function generateRule2AdjacentAngles() {
  const cx = 250;
  const cy = 150;
  const lineLength = 80;
  const angle = 55; // Use 55° for clear acute/obtuse distinction
...
// PARALLEL LINES: Teaching Diagram
function generateParallelLinesTeachingDiagram() {
  const line1Y = 100;
  const line2Y = 200;
  const lineStart = 50;
  const lineEnd = 450;

  // Transversal at 65° angle
  const transAngle = 65;
  const transX1 = 150;
  const transY1 = 40;
  const dy = 220;
  ...
  // Arc for angle at top intersection (acute - blue)
  const arcRadius = 25;
  ...
  // Arc for corresponding angle at bottom intersection (acute - blue)
  ...
  // Label positions
  const label1Pos = polarToCartesian(int1.x, int1.y, 42, transDownAngle / 2);
  const label2Pos = polarToCartesian(int2.x, int2.y, 42, transDownAngle2 / 2);
  ...
  return `<svg width="500" height="300" viewBox="0 0 500 300" xmlns="http://www.w3.org/2000/svg" style="background: white; max-width: 100%; height: auto;">
    ...
    <!-- Arcs for corresponding angles (both blue) -->
    ...
    <circle cx="${label2Pos.x.toFixed(1)}" cy="${label2Pos.y.toFixed(1)}" r="18" fill="#3b82f6"/>
    <text x="${label2Pos.x.toFixed(1)}" y="${label2Pos.y.toFixed(1)}" font-family="Arial, sans-serif" font-size="16" font-weight="bold" fill="white" text-anchor="middle" dominant-baseline="middle">a</text>
</svg>`;
}

async function fixAllTeachingDiagrams() {
  console.log('🔧 Fixing ALL teaching diagrams with accurate geometry...\n');
  ...
  // FIX RULE 1
  console.log('📝 Fixing Rule 1: Vertical Angles...\n');
  const rule1Diagram = generateRule1VerticalAngles();
  ...
  if (rule1Match) {
    ...
    console.log('  ✅ Rule 1 diagram replaced\n');
  } else {
    console.log('  ⚠️  Could not find Rule 1 diagram to replace\n');
  }

  // FIX RULE 2
  console.log('📝 Fixing Rule 2: Adjacent Angles...\n');
  const rule2Diagram = generateRule2AdjacentAngles();
  ...
  if (rule2Match) {
    ...
    console.log('  ✅ Rule 2 diagram replaced\n');
  } else {
    console.log('  ⚠️  Could not find Rule 2 diagram to replace\n');
  }

  // FIX PARALLEL LINES SECTION
  console.log('📝 Fixing Parallel Lines section...\n');
  const parallelDiagram = generateParallelLinesTeachingDiagram();
  ...
  if (parallelMatch) {
    ...
    console.log('  ✅ Parallel lines diagram replaced\n');
  } else {
    console.log('  ⚠️  Could not find Parallel lines diagram to replace\n');
  }
  ...
  console.log('✅ All teaching diagrams fixed!\n');
  console.log('📊 Summary:');
  console.log('  ✅ Rule 1: Vertical angles with accurate 70° acute and 110° obtuse');
  console.log('  ✅ Rule 2: Adjacent angles with accurate 55° and 125°');
  console.log('  ✅ Parallel Lines: Accurate 65° transversal with corresponding angles');
  console.log('  ✅ All use proper trigonometry');
  console.log('  ✅ Color-coded: Blue (acute/equal), Red (obtuse/adjacent)\n');

  return true;
}

async function main() {
  const success = await fixAllTeachingDiagrams();

  console.log('='.repeat(70));
  if (success) {
    console.log('\n✨ All teaching diagrams are now perfect!\n');
  } else {
    console.log('\n❌ Error occurred.\n');
  }
}
