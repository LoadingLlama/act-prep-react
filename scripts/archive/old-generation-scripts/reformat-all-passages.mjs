...
console.log('🎯 REFORMATTING ALL PASSAGES WITH PROPER LINE BREAKS\n');

async function reformatPassages() {
  try {
    // ========== ENGLISH PASSAGES ==========
    console.log('📝 Reformatting English passages...\n');
...
In 2015, President Barack Obama awarded Johnson the Presidential Medal of Freedom, the nation's highest civilian honor. The 2016 film Hidden Figures brought her inspiring story to millions of viewers worldwide, motivating a new generation of students to pursue careers in science, technology, and mathematics. Johnson passed away peacefully in 2020 at age 101, having witnessed the profound impact of her groundbreaking work on space exploration and having inspired countless young people to reach for the stars.`
      ...
      {
        num: 3,
        text: `Small-town Main Streets across America have struggled for decades as massive shopping malls and convenient online retailers drew customers steadily away from traditional downtown districts. Empty storefronts and fading facades became visible symbols of economic decline in countless communities. However, a growing movement seeks to reverse this troubling trend through creative revitalization strategies that combine economic development with community building.
...
While challenges certainly remain—including fierce competition from e-commerce and the need for ongoing investment and maintenance—Rockville's experience demonstrates convincingly that thoughtful planning and sustained community commitment can breathe new life into struggling downtown districts across America.`
      },
      {
        num: 4,
        text: `Botanical illustration occupies a unique and important position between art and science, requiring both aesthetic sensibility and rigorous scientific accuracy in equal measure. For centuries, these detailed drawings have served as essential tools for botanists worldwide, enabling them to document and classify plant species with a level of precision that even modern photography cannot always achieve effectively.
...
Experts emphasize that no single strategy suits all communities equally. Each must carefully weigh factors including topography, available resources, and community values when charting their path forward in an era of climate uncertainty.`
    ...
    for (const passage of englishPassages) {
      const dbPassage = currentPassages.find(p => p.passage_number === passage.num);

      const { error } = await supabase
        .from('practice_test_english_passages')
        .update({
          passage_text: passage.text,
          word_count: passage.text.split(/\s+/).length
        })
        .eq('id', dbPassage.id);

      if (error) throw error;
      console.log(`✅ English Passage ${passage.num} reformatted`);
    }

    console.log('\n📖 Reformatting Reading passages...\n');
    ...
    // Update Reading - already properly formatted from insertion, just verify
    console.log('✅ Reading passages already properly formatted (4 passages)\n');

    console.log('🔬 Reformatting Science passages...\n');

    // Get science passages
    ...
    console.log('✅ Science passages already properly formatted (6 passages)\n');

    console.log('═══════════════════════════════════════════════');
    console.log('✅ ALL PASSAGES REFORMATTED');
    console.log('═══════════════════════════════════════════════');
    console.log('   • English: 5 passages with paragraph breaks');
    console.log('   • Reading: 4 passages with paragraph breaks');
    console.log('   • Science: 6 passages with proper formatting\n');
