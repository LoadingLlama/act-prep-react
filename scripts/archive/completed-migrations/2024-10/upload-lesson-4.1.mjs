...
async function uploadLesson() {
  console.log('\n📚 UPLOADING LESSON 4.1: SYSTEMS OF EQUATIONS');
  ...
  const htmlPath = resolve(__dirname, '../docs/LESSON_4_1_SYSTEMS_OF_EQUATIONS.html');
  ...
  let { data: section, error: getSectionError } = await supabase
    ...
    .eq('section_key', 'systems-equations-main')
    .single();
  ...
  if (!section) {
    ...
    const { data: newSection, error: sectionError } = await supabase
      ...
      .insert({
        lesson_id: lesson.id,
        section_key: 'systems-equations-main',
        title: 'Main Content',
  ...
  console.log('\n' + '='.repeat(80));
  console.log('✅ LESSON 4.1 UPLOADED SUCCESSFULLY!\n');
}
