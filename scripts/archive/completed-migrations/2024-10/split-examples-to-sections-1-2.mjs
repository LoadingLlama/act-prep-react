...
// Teaching content WITHOUT examples (examples will be separate sections)
...
// Example 1 - separate section
const example1 = `<h4 style="margin: 0 0 1rem 0; padding-left: 0.75rem; border-left: 4px solid #b91c1c; color: #000000; font-weight: 700;">Example 1: Basic Substitution</h4>
...
<span style="font-family: 'Times New Roman', Times, Georgia, serif;">A. <em>x</em><sup>2</sup> + 15</span><br>
<span style="font-family: 'Times New Roman', Times, Georgia, serif;">B. <em>x</em><sup>2</sup> + 8<em>x</em> + 8</span><br>
<span style="font-family: 'Times New Roman', Times, Georgia, serif;">C. <em>x</em><sup>2</sup> + 8<em>x</em> + 15</span><br>
<span style="font-family: 'Times New Roman', Times, Georgia, serif;">D. 2<em>x</em> + 8</span><br>
<span style="font-family: 'Times New Roman', Times, Georgia, serif;">E. <em>x</em><sup>2</sup> + 15<em>x</em></span>
</p>
...
// Teaching content part 2
const teachingContent2 = `<h3 style="margin-top: 5rem; margin-bottom: 0.75rem; font-weight: 700;">2. The Substitution Process</h3>
...
// Example 2
const example2 = `<h4 style="margin: 0 0 1rem 0; padding-left: 0.75rem; border-left: 4px solid #b91c1c; color: #000000; font-weight: 700;">Example 2: Percent Problem with Substitution</h4>
...
// Example 3
const example3 = `<h4 style="margin: 0 0 1rem 0; padding-left: 0.75rem; border-left: 4px solid #b91c1c; color: #000000; font-weight: 700;">Example 3: Variables in Answer Choices</h4>
...
const teachingContent3 = `<h3 style="margin-top: 5rem; margin-bottom: 0.75rem; font-weight: 700;">3. Common Pitfalls and ACT Tips</h3>
...
async function splitToSections() {
  console.log('Splitting lesson 1.2 into text + example sections (like 1.1)...\n');
  ...
  console.log('Creating new section structure...');

  // Section 0: Intro + Section 1 (text)
  const { data: section0 } = await supabase
    .from('lesson_sections')
    .insert({
      lesson_id: lessonId,
      section_key: 'substitution-intro',
      title: 'Introduction',
      section_type: 'text',
      order_index: 0
    })
  ...
  await supabase.from('section_content').insert({
    section_id: section0.id,
    content_type: 'html',
    content: teachingContent,
    order_index: 0
  });

  // Section 1: Example 1 (example)
  const { data: section1 } = await supabase
    .from('lesson_sections')
    .insert({
      lesson_id: lessonId,
      section_key: 'substitution-example-1',
      title: 'Example 1',
  ...
  // Section 2: Section 2 teaching (text)
  const { data: section2 } = await supabase
    .from('lesson_sections')
    .insert({
      lesson_id: lessonId,
      section_key: 'substitution-process',
      title: 'The Substitution Process',
      section_type: 'text',
      order_index: 2
    })
    .select()
  ...
  await supabase.from('section_content').insert({
    ...
    content_type: 'html',
    content: teachingContent2,
    order_index: 0
  });

  // Section 3: Example 2 (example)
  const { data: section3 } = await supabase
    .from('lesson_sections')
    .insert({
      lesson_id: lessonId,
      section_key: 'substitution-example-2',
      title: 'Example 2',
      section_type: 'example',
      order_index: 3
    })
  ...
  // Section 4: Example 3 (example)
  const { data: section4 } = await supabase
    .from('lesson_sections')
    .insert({
      lesson_id: lessonId,
      section_key: 'substitution-example-3',
      title: 'Example 3',
      section_type: 'example',
      order_index: 4
    })
  ...
  // Section 5: Final content (text)
  const { data: section5 } = await supabase
    .from('lesson_sections')
    .insert({
      lesson_id: lessonId,
      section_key: 'substitution-tips',
      title: 'Tips and Takeaways',
      section_type: 'text',
      order_index: 5
    })
    .select()
  ...
  await supabase.from('section_content').insert({
    ...
    content_type: 'html',
    content: teachingContent3,
    order_index: 0
  });

  console.log('\n✅ SUCCESS! Lesson 1.2 now structured like 1.1:');
  console.log('  - 3 text sections (teaching)');
  console.log('  - 3 example sections (interactive)');
  console.log('  - Examples will be clickable like in 1.1!');
  console.log('\nRefresh your browser!');
}

splitToSections().catch(console.error);
