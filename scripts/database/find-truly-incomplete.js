...
async function findTrulyIncomplete() {
  console.log('Finding TRULY incomplete questions (referencing content not shown)...\n');
  ...
  for (const lessonKey of allEnglishLessons) {
    ...
    for (const q of questions) {
      ...
      // ONLY flag questions that reference things WITHOUT showing them
      ...
      const refsSentencesWithoutShowing = (lowText.includes('sentence 1') || lowText.includes('sentence 2') || lowText.includes('sentence 3')) && !text.includes('[Sentence');
      const refsMarshall = lowText.includes('marshall');
      const refsManta = lowText.includes('manta') && text.length < 250;

      if (refsParagraphWithoutShowing || refsUnderlinedWithoutShowing || refsSentencesWithoutShowing || refsMarshall || refsManta) {
        trulyIncomplete.push({
          ...
          issue: refsParagraphWithoutShowing ? 'refs essay/para without showing' :
                 ...
                 refsSentencesWithoutShowing ? 'refs sentences without showing' :
  ...
  trulyIncomplete.forEach(q => {
    ...
    console.log(`  ${q.title} (${q.length} chars)`);
    console.log(`  ID: ${q.id}`);
    console.log('');
  });
