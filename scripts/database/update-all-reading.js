...
async function updateReadingExplanations() {
  const explanations = JSON.parse(fs.readFileSync('all-reading-explanations.json', 'utf8'));
  ...
  for (const [questionNum, explanation] of Object.entries(explanations)) {
    try {
      ...
      if (error) {
        ...
        console.log(`✓ Updated Reading question ${questionNum}`);
        successCount++;
      }
    } catch (err) {
  ...
  console.log(`\n=== Reading Update Complete ===`);
  console.log(`Success: ${successCount}/40`);
  console.log(`Errors: ${errorCount}`);
}

updateReadingExplanations().catch(console.error);
