...
const EXPONENTS_LESSON_ID = 'b8c03bf0-99df-460d-be21-0015eebe7920';

// Import all batches
const batch1 = require('./exponents-questions-batch1.js');
const batch2 = require('./exponents-questions-batch2.js');
const batch3 = require('./exponents-questions-batch3.js');

const allQuestions = [...batch1, ...batch2, ...batch3];

console.log(`\n${'='.repeat(80)}`);
console.log(`EXPONENTS AND ROOTS QUESTIONS - VERIFICATION AND INSERTION`);
console.log(`${'='.repeat(80)}\n`);
console.log(`Total questions to insert: ${allQuestions.length}`);
console.log(`Lesson ID: ${EXPONENTS_LESSON_ID}\n`);
...
(async () => {
  ...
  console.log(`${'='.repeat(80)}\n`);
  console.log('STEP 2: Deleting existing Exponents and Roots questions...\n');

  // Delete existing questions for this lesson
  const { data: deleted, error: deleteError } = await supabase
    .from('lesson_examples')
    .delete()
    .eq('lesson_id', EXPONENTS_LESSON_ID);
  ...
  for (const q of allQuestions) {
    const record = {
      lesson_id: EXPONENTS_LESSON_ID,
      position: q.position,
      title: `Exponents and Roots Practice ${q.position}`,
