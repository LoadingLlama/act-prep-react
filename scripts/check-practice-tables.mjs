...
console.log('📋 CHECKING PRACTICE_ TABLES FOR TEST 1\n');
console.log('='.repeat(80));

// Check practice_test_english_passages
...
if (engPassages && engPassages.length > 0) {
  engPassages.forEach(p => {
    ...
    if (p.content) {
      console.log(`  First 300 chars: ${p.content.substring(0, 300)}...`);
    }
    console.log('');
  });
} else {
...
// Check practice_test_english_questions
console.log('\n📝 PRACTICE_TEST_ENGLISH_QUESTIONS (First 10):\n');
...
if (engQuestions && engQuestions.length > 0) {
  engQuestions.forEach(q => {
    ...
    console.log(`  Choice A: ${q.choice_a || 'NO CHOICE A'}`);
    console.log(`  Choice B: ${q.choice_b || 'NO CHOICE B'}`);
    console.log(`  Choice C: ${q.choice_c || 'NO CHOICE C'}`);
    console.log(`  Choice D: ${q.choice_d || 'NO CHOICE D'}`);
...
// Check practice_test_reading_passages
...
// Check practice_test_science_passages
...
if (sciPassages && sciPassages.length > 0) {
  sciPassages.forEach(p => {
    ...
    console.log(`  Content length: ${p.content?.length || 0} chars`);
    if (p.content) {
      console.log(`  First 400 chars: ${p.content.substring(0, 400)}...`);
    }
    console.log('');
  });
} else {
