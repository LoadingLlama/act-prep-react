...
async function updateLesson() {
  ...
  const { data: allBlocks } = await supabase.from('lesson_section_content').select('id, content, order_index').eq('section_id', sections[0].id).order('order_index');

  const oldLength = allBlocks[0].content?.length || 0;

  // Update block 0 with new content
  await supabase.from('lesson_section_content').update({ content: newContent }).eq('id', allBlocks[0].id);

  // Delete any extra blocks (1, 2, 3, etc.) to avoid duplicate content
  if (allBlocks.length > 1) {
    const toDelete = allBlocks.slice(1).map(b => b.id);
    await supabase.from('lesson_section_content').delete().in('id', toDelete);
