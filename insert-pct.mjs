...
const html = readFileSync('formatted-percentages.html', 'utf-8');

const { data: lesson } = await supabase
  .from('lesson_metadata')
  .select('id')
  .eq('lesson_key', 'percentages')
  .single();
...
if (existingQuiz) {
  ...
  const { data: newQuiz } = await supabase
    ...
    .insert({
      lesson_id: lesson.id,
      title: 'Percentages Mastery Check',
      quiz_type: 'mastery',
      position: 9999,
      is_required: true
    })
...
const questions = [
  ...
  },
  {
    ...
    options: [
      ...
      { text: '3', is_correct: false, explanation: 'Percentages can have at most 2 real solutions. Here the discriminant is negative, so 0 real solutions.' },
      { text: 'Infinite', is_correct: false, explanation: 'Percentages have a finite number of solutions. Use discriminant: −16 < 0 means 0 real solutions.' }
    ]
  },
  {
...
console.log('✅ Percentages lesson complete!');
