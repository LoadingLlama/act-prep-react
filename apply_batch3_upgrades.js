...
async function applyBatch3Upgrades() {
  console.log('='.repeat(100));
  console.log('APPLYING BATCH 3 - 100 ACT-AUTHENTIC QUESTION UPGRADES');
  console.log('='.repeat(100));
  console.log();

  const data = JSON.parse(fs.readFileSync('/tmp/act_replacements_batch3_fixed.json'));

  // Handle different key names (replacements vs generated_questions)
  ...
  console.log(`  Batch 3: ${successful} questions`);
  console.log(`  Total:   ${153 + successful} / 1412 weak questions (${((153 + successful)/1412*100).toFixed(1)}%)`);
  console.log(`  Remaining: ${1412 - 153 - successful} questions`);
  console.log('='.repeat(100));
}

applyBatch3Upgrades().catch(console.error);
